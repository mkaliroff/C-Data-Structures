...
typedef int (*priority_queue_compare_func_t) (const void *data, const void *new_data);

/* Used in Erase. Should return non-zero if "data" match the "parameter" */
typedef int (*priority_queue_ismatch_func_t) (const void *data, const void *new_data);
